...
if (process.argv[2] === "dev") {
    eval(require('fs').readFileSync(__dirname + "/dev-mock.js").toString());
}
...
async function main() {
    try {
        ...
        dirs = dirs.filter(d => d.isDirectory() && d.name.startsWith("Agent"));
        let bnetPath = "";
        let w3Path = "";
        if (dirs.length > 0) {
            for (const dir of dirs.reverse()) {
                const path = baseDir + "\\" + dir.name + "\\Logs";
                if (fs.existsSync(path)) {
                    let files = fs.readdirSync(path);
                    files = files.filter(f => f.startsWith("Agent-")).reverse();
                    for (const file of files) {
                        const content = fs.readFileSync(path + "\\" + file).toString();
                        //console.log(content);
                        let result = content.match(/"([^'"]*:[^'"]*Battle.net.exe)"/);
        ...
        while (processRunning("Warcraft III.exe")) {
            showProgress("Please close Warcraft III to continue launching W3Champions");
            await sleep(1000);
        }
        ...
        while (!processRunning("Battle.Net.Exe")) {
...
async function patchingRequired() {
    try {
        ...
        return patchData.version != currentPatchHash || currentPatchHash == "" || currentPatchHash == null || currentPatchHash == undefined;
    } catch (e) {
        return false;
    }
}
...
function processRunning(win, mac, linux) {
    ...
    const cmd = plat == 'win32' ? 'tasklist /FI "STATUS eq RUNNING"' : (plat == 'darwin' ? 'ps -ax | grep ' + mac : (plat == 'linux' ? 'ps -A' : ''))
